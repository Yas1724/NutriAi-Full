...
function SocialRow() {
  ...
  return (
    <div style={{ display: "flex", gap: 10 }}>
      <button
        style={{ ...s, opacity: 0.4, cursor: "not-allowed" }}
        disabled
      ...
      >
        <svg width="15" height="15" viewBox="0 0 24 24" fill={t.socialText}>
          <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98l-.09.06c-.22.14-2.2 1.29-2.18 3.85.03 3.05 2.67 4.06 2.7 4.07-.03.07-.42 1.44-1.38 2.65M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/>
        </svg>
        Coming soon
      </button>
    </div>
...
function AppInner() {
  ...
  useEffect(() => {
    ...
    // ── Handle OAuth error query params (/? error=oauth_denied etc.) ─────────
    ...
    if (oauthError) {
      const errorMessages = {
        ...
        oauth_no_email:         "Google did not provide an email address.",
        oauth_failed:           "Google sign-in failed. Please try again.",
      };
      const msg = errorMessages[oauthError] || "Sign-in failed. Please try again.";
