...
export const User = {
    ...
    // ── createOAuth({ name, email, googleId, isVerified }, randomPassword) ────
    // Used by the OAuth controller — creates a user without a real password.
    // Also stores google_id so the same account can be linked later.
    async createOAuth({ name, email, googleId, isVerified }, randomPassword) {
        const bcrypt = await import('bcryptjs');
        const hashedPassword = await bcrypt.default.hash(randomPassword, 10);

        // Use INSERT ... ON CONFLICT to handle the edge case where a concurrent
        // signup happens at the same time as an OAuth login.
        const { rows } = await query(
            `INSERT INTO users
               (name, email, password, google_id, is_verified)
             VALUES ($1, $2, $3, $4, $5)
             ON CONFLICT (email) DO UPDATE
               SET google_id   = EXCLUDED.google_id,
                   is_verified = TRUE
             RETURNING ${ALL_COLS}`,
            [name, email, hashedPassword, googleId || null, isVerified]
        );
        return toUser(rows[0]);
    },
};
